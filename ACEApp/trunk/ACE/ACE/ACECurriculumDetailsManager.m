...
@property (retain) NSMutableDictionary *failureInfo;

@end

@implementation ACECurriculumDetailsManager

@synthesize queue;
@synthesize curriculamMasterDataManager;
@synthesize failureInfo;

- (void)dealloc 
{
    self.curriculamMasterDataManager = nil;
               ...
               delegate:(id)_delegate
{
    if (self = [super initWithDelegate:_delegate]) {
        apiReqType = eCurriculamDetails;
        self.sessionToken = token;
        self.queue = [NSMutableArray 
                      arrayWithArray:[NSArray  arrayWithObjects:student, nil]];
        self.failureInfo = [[NSMutableDictionary alloc] init];
    }
                ...
                delegate:(id)_delegate
{
    if (self = [super initWithDelegate:_delegate]) {
        apiReqType = eCurriculamDetails;
        self.sessionToken = token;
        self.queue = [NSMutableArray arrayWithArray:idList];
        self.failureInfo = [[NSMutableDictionary alloc] init];
    }
...
- (NSString*)getAPIURL
{
    ...
    if ([self.queue count] > 0) {
        ...
        apiURL = [apiURL stringByAppendingString:[NSString stringWithFormat:@"/CurriculumsDetails/%d?date=%@",studentId,
...
- (void)requestFailed:(ASIHTTPRequest *)request
{
    ...
    ACEStudent *student = [self.queue lastObject];
    [self.failureInfo setValue:student.name 
                        forKey:[NSString stringWithFormat:@"%d",[self.queue indexOfObject:student]]];
...
- (void) requestFinished: (ASIHTTPRequest *)request
{
    ...
    int resultCode = [[curriculumDict objectForKey:@"ResultCode"] intValue];
    
    if (resultCode == 200) {
        //1.IT
        NSArray *ITCurriculums = [dataDict valueForKey:@"ITCurriculums"];
        [self insertITCurriculums:ITCurriculums andActualDictionary:curriculumDict];
        
        //2. TA
        NSArray *TACurriculums = [dataDict valueForKey:@"TACurriculums"];
        [self insertTACurriculums:TACurriculums andActualDictionary:curriculumDict];
        
        //3. SA
        NSArray *SACurriculums = [dataDict valueForKey:@"SACurriculums"];
        [self insertSACurriculums:SACurriculums andActualDictionary:curriculumDict];
    }else{
        //Base classes override this to modify error. By default it passes error to delegate.
        ACEStudent *student = [self.queue lastObject];
        [self.failureInfo setValue:student.name 
...
//Insert curriculum details into DB.
...
- (void)insertTACurriculums:(NSArray*)curriculumArray andActualDictionary:(NSDictionary*)rootDict
{
    for (NSArray *crlm in curriculumArray) {
        for (NSDictionary *dictionary in crlm) {
            ...
            [StuCurriculumEntry setValue:[NSNumber numberWithInt:studId] forKey:@"ACEStudentId"];
...
- (void)loadCurriculamMasterData
{
    ...
    self.curriculamMasterDataManager = [[ACEMasterDataManager alloc] initWithToken:sessionToken 
                                                                          delegate:self];
...
- (void)ACEConnectionManager:(ACEDataManager*)_manager didFinishLoadingData:(NSMutableArray*)responseDataValue{
    ...
    if(_manager.requestType == eMasterData){
        [self updateComplete];
    }
}
...
{
    if(manager.requestType == eMasterData){
        [self updateComplete];
    }
}
...
- (void)updateComplete;
{
    if ([self.delegate respondsToSelector:@selector(ACECurriculamDetailsManagerDidREcieveAllCurriculum:withFailureDictionary:)]) {
        NSMutableDictionary *errorDict = ([[self.failureInfo allKeys] count] > 0) ? self.failureInfo : nil;
        [self.delegate ACECurriculamDetailsManagerDidREcieveAllCurriculum:self withFailureDictionary:errorDict];
    }
}
